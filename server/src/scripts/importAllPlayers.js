...
function isSenior(age, category) {
  // Senior if age is 19 or older, OR category contains "senior"
...
async function importPlayers() {
  try {
    ...
    console.log(`Starting indices: Juniors at ZTAJ${String(juniorStartIndex).padStart(4, '0')}, Seniors at ZTAS${String(seniorStartIndex).padStart(4, '0')}\n`);
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      try {
        ...
        // Age <= 18 = junior, Age >= 19 = senior (OR category says senior)
        ...
        // Check if user already exists by first name, last name, and age
        ...
        if (existingUser) {
          console.log(`Skipping ${firstName} ${lastName} (Age: ${age}) - already exists`);
          skipped++;
          // Decrement the counter since we didn't use this ZPIN
          if (isPlayerSenior) {
            seniorCount--;
          } else {
            juniorCount--;
        ...
        const playerGender = gender?.toLowerCase().includes('female') ||
                             gender?.toLowerCase().includes('girl') ||
                             gender?.toLowerCase().includes('women') ||
                             gender?.toLowerCase().includes('ladies')
          ? 'female'
          : 'male';
        ...
        const playerType = isPlayerSenior ? 'Senior' : 'Junior';
        console.log(`✓ Imported ${firstName} ${lastName} (Age: ${age}, ${playerType}) - ${zpin}`);
        imported++;
